...
var PeterpanMap = function () {
    ...
    _createClass(PeterpanMap, [{
        ...
        value: function getInitialRouteParams(options) {
            ...
            var that = this;
            var localQuery;
            if (window.localStorage) localQuery = localStorage.getItem('peterpanQuery');
        ...
        value: function renderList(list) {
            ...
            function generateView(house) {
                ...
                var convertedTemplate = template.replace('%HOUSE_ID', hidx).replace('%IMAGE_PATH', img_path).replace('%HOUSE_PRICE', function () {
                    var str = '';
        ...
        value: function getMapInfo() {
            var _map$getBounds = this._map.getBounds(),
                ba = _map$getBounds.ba,
                fa = _map$getBounds.fa,
                ga = _map$getBounds.ga,
                ha = _map$getBounds.ha;

            return {
                ba: Number(ba).toFixed(7),
                fa: Number(fa).toFixed(7),
                ga: Number(ga).toFixed(7),
        ...
        value: function setAutoComplete() {
            ...
            var elem = dom.autocomplete({
                ...
                source: function source(req, res) {
                    _this5.getAutoCompleteSource(req.term, res);
                },
                ...
                select: function select(event, _ref7) {
                    ...
                    if (item.cb) return item.cb();else return _this5.moveTo(item.location || item.locatoin);
                },
        ...
        value: function _setFilterToggle() {
            ...
            return function () {
                filters.change(function (event) {
                    _this6.handleFilter($(event.currentTarget).attr('id'));
                });
                ...
                selectors.change(function (event) {
                    ...
                    switch (target.attr('name')) {
                        case 'deposit_1':
                            rightValue = _this6._query.checkDeposit[1];
                            if (rightValue !== 999) {
                                _this6._query.checkDeposit[0] = Math.min(rightValue, targetValue);
                                target.val(_this6._query.checkDeposit[0]);
                            } else {
                                _this6._query.checkDeposit[0] = targetValue;
                            }
                            break;
                        case 'deposit_2':
                            leftValue = _this6._query.checkDeposit[0];
                            if (leftValue !== 999) {
                                _this6._query.checkDeposit[1] = Math.max(leftValue, targetValue);
                                target.val(_this6._query.checkDeposit[1]);
                            } else {
                                _this6._query.checkDeposit[1] = targetValue;
                            }
                            break;
                        case 'monthly_1':
                            rightValue = _this6._query.checkMonth[1];
                            if (rightValue !== 999) {
                                _this6._query.checkMonth[0] = Math.min(rightValue, targetValue);
                                target.val(_this6._query.checkMonth[0]);
                            } else {
                                _this6._query.checkMonth[0] = targetValue;
                            }
                            break;
                        case 'monthly_2':
                            leftValue = _this6._query.checkMonth[0];
                            if (leftValue !== 999) {
                                _this6._query.checkMonth[1] = Math.max(leftValue, targetValue);
                                target.val(_this6._query.checkMonth[1]);
                            } else {
                                _this6._query.checkMonth[1] = targetValue;
                            }
                            break;
                    }
                ...
                resetElem.on('click', function () {
                    var query = _this6.getDefaultQuery();
                    var _query2 = _this6._query,
        ...
        value: function getHouses(isAdd) {
            ...
            $.ajax(URL).then(function (res) {
                if (!_.isEmpty(res)) {
                    res = JSON.parse(res);
                    var ret = res.list && res.list[_this7._query.category] && res.list[_this7._query.category].data;
                    if (!_.isEmpty(ret)) {
                        if (isAdd) {
                            _this7.data = [].concat(_toConsumableArray(_this7.data), _toConsumableArray(ret));
                        } else {
                            _this7.data = ret;
                            $('.houseLists').scrollTop(0);
                        }
                    } else {
                        if (isAdd) _this7._query.pageIndex--;else _this7.data = [];
                    }
                }
            ...
            $.ajax(this.SUBWAY_COLLEGE).then(function (res) {
                if (res) {
                    _this7.generateMarkers(JSON.parse(res));
                }
            });
        }
    }, {
        ...
        value: function generateMarkers(_ref9) {
            ...
            subway.forEach(function (s) {
                ...
                var custom = new daum.maps.CustomOverlay({
                    position: new daum.maps.LatLng(latitude, longitude),
                    content: '<div class="subwayMarker markerSubway"><i class="subwayIcon peterpan-subway"></i><div>' + station + '\uC5ED</div></div>'
                });

                custom.setMap(map);
                _this8._markers.push(custom);
            });
            ...
            university.forEach(function (u) {
                ...
                var custom = new daum.maps.CustomOverlay({
                    position: new daum.maps.LatLng(latitude, longitude),
                    content: '<div class="collegeMarker">' + name + '</div>'
                });
        ...
        value: function scrollTo() {
            ...
            // let pos = 0;
            //
            // const container = $('.housesContainer').eq(index);
            // container.addClass('containerFixed');
            // const indicator = $('.housesIndicator').eq(index);
            // pos = container.position().top;
            // const parent = $('.houseLists').eq(0);
            // parent.animate({
            //     scrollTop: pos || 1
            // }, 150);
        ...
        value: function generateCategories() {
            ...
            parentElem.height(HEIGHT);
            $('.houseLists').css('top', HEIGHT + 87);
            ...
            $('.categoryWrapper').delegate('li', 'click', function (event) {
                ...
                var value = elem.text();
                if (value === _this11._query.category) return;

                _this11._query.category = value;
                $('.categorySelected').removeClass('categorySelected');
                elem.addClass('categorySelected');
                _this11.setRouteParams();
            });
        ...
        get: function get() {
            ...
            var filters = 'filter=latitude:' + info.ha + '~' + info.ga + '||longitude:' + info.ba + '~' + info.fa;
            ...
            var _map$getCenter = this._map.getCenter(),
                hb = _map$getCenter.hb,
                ib = _map$getCenter.ib;

            filters += '&center={"latitude": ' + ib + ', "longitude": ' + hb + '}';
        ...
        get: function get() {
            var info = this.getMapInfo();
            return 'http://api.seoul.peterpanz.com/info?filter=latitude:' + info.ha + '~' + info.ga + '||longitude:' + info.ba + '~' + info.fa;
        }
    }]);
...
var PeterpanPrice = function () {
    ...
    _createClass(PeterpanPrice, [{
        ...
        key: "generateRadioBtns",
        value: function generateRadioBtns() {
            var _this = this;

            var TEMPLATE = "\n            <div class=\"radioContainer\">\n                <div>\n                    \uC804\uC138\n                    <input type=\"radio\" name=\"type\" value=\"\uC804\uC138\" " + (this._rentType === "전세" ? "checked" : "") + ">\n                </div>\n                <div>\n                    \uC6D4\uC138\n                    <input type=\"radio\" name=\"type\" value=\"\uC6D4\uC138\"  " + (this._rentType === "월세" ? "checked" : "") + ">\n                </div>\n            </div>\n        ";

            $('#priceContainer').prepend($(TEMPLATE));
            $('.radioContainer input').on('change', function (e) {
                _this.rentType = e.target.value;
            });
        }
    }, {
        key: "getData",
        value: function getData() {
            ...
            var info = this.getMapInfo();
            $.ajax("http://api.seoul.peterpanz.com/price?filter=latitude:" + info.ha + "~" + info.ga + "||longitude:" + info.ba + "~" + info.fa + "&rentType=" + this._rentType).then(function (res) {
                if (res) {
                    res = JSON.parse(res);
                    _this2.data = res;
                }
        ...
        value: function generateMap() {
            ...
            var container = document.getElementById('map');
            var options = {
                center: new daum.maps.LatLng(37.5105634, 127.0227266),
                level: 5
            };
        ...
        value: function generateMarkers() {
            ...
            arr.forEach(function (v) {
                ...
                var custom = new daum.maps.CustomOverlay({
                    ...
                    content: "\n                    <div class=\"priceMarker\">\n                        <div class=\"title\">\n                            " + siName + "<br/>\n                            " + guName + "/" + dongName + "\n                        </div>\n                        " + getContent(_this4._rentType, price) + "\n                    </div>\n                "
                });

                custom.setMap(map);
                _this4._markers.push(custom);
            });
